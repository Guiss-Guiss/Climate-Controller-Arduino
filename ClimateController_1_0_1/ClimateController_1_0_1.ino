...
#include <DFRobot_SHT20.h>     // Library for SHT20 temperature and humidity sensor by DfRobot.
#include <DallasTemperature.h> // Library for DallasTemperature sensor by Miles Burton.
...
// Defines
#define ONE_WIRE_BUS 7 // OneWire bus pin.
...
OneWire oneWire(ONE_WIRE_BUS);              // OneWire addressing.
DallasTemperature sensors(&oneWire);        // DallasTemperature sensor addressing.
DFRobot_SHT20 sht20(&Wire, SHT20_I2C_ADDR); // SHT20 sensor addressing
...
const char a25[] PROGMEM = "  Press >> to Save  ";  // English string for "  Press >> to Save  ".
const char a26[] PROGMEM = "  Sauvegarde = >>   ";  // French string for "  Press >> to Save  ".
...
const char a46[] PROGMEM = "Heure Or Lumieres:  ";  // French string for "Lights Off Time:   ". 
...
void Relays()
{
  ...
  sensors.setResolution(12);                                          // Sets the resolution of the DallasTemperature sensor to 12 bits
  DateTime now = rtc.now();                                           // Gets the current time from the RTC module
  humidity = sht20.readHumidity();                                    // Reads the humidity from the SHT20 sensor
  sensors.requestTemperatures();                                      // Requests the temperature from the DallasTemperature sensor
  temperatureCelcius = sensors.getTempCByIndex(0);                         // Gets the temperature in Celsius from the DallasTemperature sensor
  temperatureFahrenheit = (DallasTemperature::toFahrenheit(temperatureCelcius));   // Converts the temperature in Celsius to Fahrenheit
  ...
  if (isnan(humidity) || isnan(temperatureCelcius) || isnan(temperatureFahrenheit) || temperatureCelcius == -127 || temperatureFahrenheit == 196.6)   // Checks if the temperature and humidity values are not numbers
  {
  ...
  if (currentTime - previousTime >= loopInterval)    // Update relays every 10 seconds
  {
    if (unitChoice == 1)
    {
      ...
      if (humidity < lowHumidity && temperatureFahrenheit < highTempFahrenheit)
      {
      ...
      if (humidity > highHumidity && temperatureFahrenheit < highTempFahrenheit)
      {
    ...
    else if (unitChoice == 0)
    {
      ...
      if (humidity < lowHumidity && temperatureCelcius < highTempCelcius)
      {
        digitalWrite(humLow, LOW);
      }
      else
...
void setup()
{
  ...
  sensors.setResolution(12); // Set the resolution of the sensors to 12 bits
  sht20.initSHT20();
  ...
  digitalWrite(cool, HIGH);           // Turn off the cool
  digitalWrite(humLow, HIGH);         // Turn off the low humidity
  digitalWrite(humHigh, HIGH);        // Turn off the high humidity
  digitalWrite(lights, HIGH);         // Turn off the lights
